...
namespace CHAI.LISDashboard.Modules.Admin.Views
{
    public partial class NodeEdit : Microsoft.Practices.CompositeWeb.Web.UI.Page, INodeEditView
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                Master.ShowMessage(new AppMessage("Error: You must select ASPX page." , CHAI.LISDashboard.Enums.RMessageType.Error));
                return;
            }
            ...
            try
            {
                ...
                Master.ShowMessage(new AppMessage("Node was saved successfully", CHAI.LISDashboard.Enums.RMessageType.Info));
            }
            catch (Exception ex)
            {
                Master.ShowMessage(new AppMessage("Error: Unable to save node. " + ex.Message, CHAI.LISDashboard.Enums.RMessageType.Error));
            }
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                _presenter.DeleteIt();
                Master.ShowMessage(new AppMessage("Node was deleted successfully", CHAI.LISDashboard.Enums.RMessageType.Info));
            }
            catch (Exception ex)
            {
                Master.ShowMessage(new AppMessage("Error: Unable to delete node. " + ex.Message, CHAI.LISDashboard.Enums.RMessageType.Error));
            }
        }
            ...
            foreach (RepeaterItem ri in rptRoles.Items)
            {
                ...
                if (chkView.Checked || chkEdit.Checked)
                {
                    ...
                    else
                    {
                        ...
                        int a = Convert.ToInt32(ViewState[ri.UniqueID]);
                        NodeRole np = node.GetNodeRole(Convert.ToInt32(ViewState[ri.UniqueID]));
                        if(np != null) {
                            np.ViewAllowed = chkView.Checked;
                            np.EditAllowed = chkEdit.Checked;
                        }
