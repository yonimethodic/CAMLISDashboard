    ...
    public partial class TabEdit : Microsoft.Practices.CompositeWeb.Web.UI.Page, ITabEditView
    {
        ...
        {           
            ...
            int id = _presenter.SaveOrUpdateTab();
                ...
                {
                    Master.TransferMessage(new AppMessage("Tab was saved successfully", CHAI.LISDashboard.Enums.RMessageType.Info));
                    string url = String.Format("~/Admin/TabEdit.aspx?{0}=0&{1}={2}", AppConstants.TABID, AppConstants.NODEID, id);
                    _presenter.Navigate(url);
                }
                else
                    Master.ShowMessage(new AppMessage("Tab was saved successfully", CHAI.LISDashboard.Enums.RMessageType.Info));
            //}
            //catch (Exception ex)
            //{
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                _presenter.DeleteIt();
                Master.TransferMessage(new AppMessage("Tab was deleted successfully", CHAI.LISDashboard.Enums.RMessageType.Info));
                _presenter.CancelIt();
            }
            catch (Exception ex)
            {
                Master.ShowMessage(new AppMessage("Error: Unable to delete Tab. " + ex.Message, CHAI.LISDashboard.Enums.RMessageType.Error));
            }
        }
