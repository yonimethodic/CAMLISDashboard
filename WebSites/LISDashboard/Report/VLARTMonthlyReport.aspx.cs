...
{
    public partial class VLARTMonthlyReport : POCBasePage, IDefaultView
    {
        ...
        public override string PageID
        {
            ...
            get
            {
                return "{bfaaa4c2-38f6-478f-9ff0-5cf365999497}";
            }
        }
        [CreateNew]
        ...
        private void PopulateYears()
        {
            ...
            ddlYear.SelectedValue = startYear.ToString();
        }
        ...
        private void LoadReport()
        {
            ...
            {                
                ...
                PrDateDescription = ddlReportShowing.SelectedItem.ToString();               
                ...
                //dtVLSummaryOT.TableName = "VLARTMonthlyReport";

                string path = string.Empty;
                DataTable dtVLSummaryOT = null;
                DataSet ds = new DataSet();
                if(rdbViewOption.SelectedValue == "StateRegion")
                {
                    var dsResult = dao.GetRptARTMonthly(CurrentUser.Id, CurrentUser.AppUserRoles[0].Role.Name,
                            Convert.ToInt32(ddlYear.SelectedValue), Convert.ToInt32(ddlMonth.SelectedValue), ProvinceId, FacilityType1, ddlLab.SelectedValue, ddlReportShowing.SelectedValue);
                    path = Server.MapPath(Convert.ToString("~/Report/VLARTMonthlyReport.rpt"));

                    dtVLSummaryOT = dsResult.Tables[0].Copy();
                    dtVLSummaryOT.TableName = "VLARTMonthlyReport";
                }
                else if (rdbViewOption.SelectedValue == "Indicator")
                {
                    var dsResult = dao.GetRptARTMonthlyByIndicators(CurrentUser.Id, CurrentUser.AppUserRoles[0].Role.Name,
                            Convert.ToInt32(ddlYear.SelectedValue), Convert.ToInt32(ddlMonth.SelectedValue), ProvinceId, FacilityType1, ddlLab.SelectedValue);
                    path = Server.MapPath(Convert.ToString("~/Report/VLARTMonthlyByIndicatorsReport.rpt"));

                    dtVLSummaryOT = dsResult.Tables[0].Copy();
                    dtVLSummaryOT.TableName = "VLARTMonthlyByIndicatorsReport";
                }                
                ...
                rprt.SetParameterValue("PrDate", ddlMonth.SelectedItem + "/" + ddlYear.SelectedItem);
                rprt.SetParameterValue("PrLab", ddlLab.SelectedIndex == 0 ? string.Empty : ddlLab.SelectedItem.ToString());
                rprt.SetParameterValue("PrReportDescription", PrDateDescription);                            
        ...
        //added by wwp
        protected string ChangeDateFormat(String dString)
        {
            if (dString != string.Empty)
            {
                DateTime Ndatetime = new DateTime();
                Ndatetime = ConvertToDateTime(dString);
                dString = Ndatetime.ToString("MM/dd/yyyy");
            }
            return dString;
        }
